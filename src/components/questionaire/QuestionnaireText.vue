<template>
  <div>
    <div class="mb-2 mr-sm-2 mb-sm-0" v-if="hasStyle">
      ...
      <h1 v-else-if="data.type=='title'" :style="computedStyle"> {{ content }} </h1>
      <a v-else-if="data.type=='link'" :href="data.target" target="_blank" :style="computedStyle"> {{ content }} </a>
    </div>    
    <div class="mb-2 mr-sm-2 mb-sm-0" v-else> 
  ...
  export default {
    ...
    computed: {
      ...
      { 
        ...
        return this.data.style ? Object.entries(this.data.style).map(([k, v]) => `${k}:${v}`).join(';') : "" // empty if there is no style field.
      },
      hasStyle()
      {        
