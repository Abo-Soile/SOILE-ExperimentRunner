<template>
    <Dialog v-model:visible="dialogVisible" modal header="Set Study Properties" :style="{ width: '50vw' }">
        ...
        <StudyProperties v-model:name="name" 
        v-model:descriptionShort="descriptionShort"
            ...
            @update:valid="(event) => dataValid = event"></StudyProperties>
        <template #footer>
            <Button label="Cancel" icon="pi pi-times" @click="$emit('selected', false)" text />
            <Button label="Create" icon="pi pi-check" @click="$emit('selected', selectionValid ? getProjectData() : false)"
                autofocus :disabled="!selectionValid" />
        </template>
    </Dialog>
</template>
