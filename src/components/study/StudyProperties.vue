<template>
    ...
    <div class="grid">
        ...
        <div class="col">
            <div>
                <InputText id="shortcut" :class="{ 'p-invalid': !shortCutValid }" v-model="currentShortCut" :maxlength="15"/>
            </div>
            <div>
                <small v-if=!shortCutValid class="p-error" id="text-error">Shortcut can be max 15 characters and needs to be
                    alphanumeric</small>
            </div>
        </div>
    </div>
...
export default {
    ...
    emits: ['update:valid','update:name', 'update:shortCut','update:descriptionShort','update:descriptionLong','update:private'],
    props: {
        name: {
            type: String,
            required: true
    ...
    computed: {
        ...
        shortCutValid()
        {
            const isValid = ((this.shortCut.search(/^[0-9a-zA-Z]*$/) == 0) || this.shortCut.length == 0 ) && this.shortCut.length <= 15
