    ...
    actions: {
        ...
        */
        async getTagsForElement(uuid, type) {
            ...
            switch (usedType) {
                case "task": options = await this.getTaskOptions(uuid); break;
                case "project": options = await this.getProjectOptions(uuid); break;
                case "experiment": options = await this.getExperimentOptions(uuid); break;
            }
            return options.filter((x) => x.tag).map(((x) => { return { tag: x.tag, version: x.version } }))
        },
        /**
        ...
        async createElement(name, data, type) {            
            try {
                const response = await axios.post("/" + type.toLowerCase() + "/create", null, {params: {name: name}})
        ...
        async addFileToTask(uuid, version, filename, file)
        {
            try {
                ...
                const response = await axios.post('/task/' + uuid + "/" + version + "/resource/" + filename, formData );                
                return response.data.version;
            }
            catch (e) {
                console.log("Error" + e);
