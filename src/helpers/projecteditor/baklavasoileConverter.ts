...
export async function BaklavaToSoileProjectJSON(graph: Graph): Promise<SOILEProject> {
    ...
    for (const node of nodes) {
        ...
        {
            throw new Error("Cannot convert. Node " + soileNode.title + " is not valid (Probably no target Element selected)")
        }
    ...
    for (const node of nodes) {
        ...
        if (node.type === "TaskNode") {
            if (!(node as TaskNode).isStartNode()) {
                ...
                experimentData.elements.splice(0, 0, { elementType: "task", data: createTaskJson(node as TaskNode, nextMap) });
            }
        }
        if (node.type === "FilterNode") {
...
function createFilterJson(cnode: FilterNode, nextMap: Map<string, string>): FilterInstance {
    ...
    const filterData = {
        instanceID: cnode.title,
        defaultOption: nextMap.get(cnode.outputs.default.id) ? nextMap.get(cnode.outputs.default.id) : "end",
        options: filters,
        position: cnode.position
    }
    return filterData;
