<template>
    <div v-if="isRunning && codeReady">
        <SoileExpRunner v-if="codeType == 'elang'" :code="compiledCode" :outputs="outputs"
            @handleSubmit="event => displayResults(event)" @handleError="error => handleError(error)"
            @handleUpload="event => uploadFile(event.file, event.fileName, event.idCallBack, event.errorCallBack)">
        </SoileExpRunner>
        <PsychoJsRunner v-if="codeType == 'psychopy'" :code="compiledCode" :psychoJSVersion="codeTypeVersion"
            @handleSubmit="event => submitResults(event)" @handleError="error => handleError(error)"
            @handleUpload="event => uploadFile(event.file, event.fileName, event.idCallBack, event.errorCallBack)">
        </PsychoJsRunner>
        <SoileQuestionnaire v-if="codeType == 'qmarkup'" :code="compiledCode" :outputs="outputs"
            @handleSubmit="event => submitResults(event)" @handleError="error => handleError(error)"></SoileQuestionnaire>
